...
vec3 compute_light(Ray camera_ray, vec3 pos, vec3 normal, vec3 albedo) {
    vec3 L = normalize(lightPos - pos); // direction to light

    Ray ray = Ray(pos + normal * 0.1, L);
    Hit hit = dda(ray, MAX_STEPS * 5);
    // Ambient from sky
    ...
    if (hit.hit) {
        ...
        vec3 lightColor = vec3(20.0, 18.0, 15.0) * 5000.0;
        float shininess = 60;
...
void main() {
    ...
    if (intersectAABB(camera_ray, boxMin, boxMax, t)) {
        ...
        if (hit.hit) {
            ...
            vec3 color = compute_light(camera_ray, hit.position, hit.normal, albedo);
